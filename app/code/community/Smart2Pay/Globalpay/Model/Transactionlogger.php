...
class Smart2Pay_Globalpay_Model_Transactionlogger extends Mage_Core_Model_Abstract
{
    ...
    static function validateTransactionLoggerExtraParams( $params_arr, $extra_arr = false )
    {
        ...
        if( empty( $extra_arr['keep_default_values'] ) )
            $extra_arr['keep_default_values'] = false;

        $default_values = self::defaultTransactionLoggerExtraParams();
        $new_params_arr = array();
        foreach( $default_values as $key => $def_val )
        {
            if( !array_key_exists( $key, $params_arr )
             or is_null( $params_arr[$key] ) )
                continue;

            if( is_int( $def_val ) )
                $new_val = intval( $params_arr[$key] );
            elseif( is_string( $def_val ) )
    ...
    public function write( $transaction_arr, $extra_params = null )
    {
        /** @var Smart2Pay_Globalpay_Helper_Helper $s2pHelper */
        $s2pHelper = Mage::helper( 'globalpay/helper' );
        /** @var Smart2Pay_Globalpay_Model_Logger $s2pLogger */
        ...
        try
        {
            $table_name = $this->getResource()->getTable( 'globalpay/transactionlogger' );
            ...
            and ($existing_id = $conn_read->fetchOne( 'SELECT id FROM `'.$table_name.'` WHERE merchant_transaction_id = \''.$s2pHelper->prepare_data( $transaction_arr['merchant_transaction_id'] ).'\' LIMIT 0, 1' )) )
            {
                ...
                $conn_write->update( $table_name, $insert_arr, 'id = \''.$existing_id.'\'' );
            } else
            {
                ...
                $conn_write->insert( $table_name, $insert_arr );
            }
        } catch ( Exception $e )
        {
            $s2pLogger->write( 'Exception ('.$e->getMessage().')', 'trans_logger' );
